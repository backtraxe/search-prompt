#include "TrieAsync.hpp"

AsyncNode::AsyncNode() : leaf_(false), is_deleted_(false) {
...
AsyncTrie::AsyncTrie() { root = std::make_unique<AsyncNode>(); }
    ...
    return std::async(std::launch::async, [&]() -> result_t {
        ...
        for (std::size_t i = 0; i < SIZE; ++i) {
            ...
            if (tmp != nullptr && !tmp->is_deleted_)
                this->search_rec(word + std::string(1, i + 'a'), tmp, '\0',
                                 i + 'a', nullptr, row, row_size + 1, query,
                                 result);
        }
        return result;
    });
...
void AsyncTrie::search_rec(std::string word, AsyncNode *curr,
    ...
    if (min_elt < result.second) {
        for (std::size_t i = 0; i < SIZE; ++i) {
            ...
            if (tmp != nullptr && !tmp->is_deleted_)
                this->search_rec(word + std::string(1, i + 'a'), tmp,
                                 prev_letter, i + 'a', prev_row, row, size,
                                 query, result);
        }
    }
}
...
std::future<void> AsyncTrie::insert(const std::string &w) {
    ...
    return std::async(std::launch::async, [&]() -> void {
        ...
        for (const char c : w) {
            ...
            if (child == nullptr)
                curr->children_[c - 'a'].compare_exchange_strong(
                    child, new AsyncNode());
