...
class AsyncTrie : public IAsyncDictionary {
  public:
    ...
    void search_rec(std::string, AsyncNode *, const char, const char, int *,
                    int *, std::size_t, const std::string &, result_t &) const;
...
template <class Iterator>
AsyncTrie::AsyncTrie(Iterator begin, Iterator end) : AsyncTrie() {
    for (; begin != end; begin++)
